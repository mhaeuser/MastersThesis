...
{
  name={bit-packing},
  description={a technique to store data in as few bits as possible without changing its encoding format}
}
...
\newglossaryentry{data-structure-padding}
{
  ...
  description={a technique to insert \glspl{byte} between data structure members or at its end to conform to \gls{data-alignment} requirements}
}
...
\newglossaryentry{pseudo-randomization}
{
  ...
  description={a technique to produce values that are close enough to statistically random values by deterministic means}
}

\newglossaryentry{ASLRg}
{
  name={Address Space Layout Randomization},
  description={a technique to make the addresses of process components, such as the executable \gls{image}, \gls{call-stack}, \gls{heap-memory}, or \glspl{shared-library}, unpredictable by the means of \gls{pseudo-randomization}}
}
...
\newglossaryentry{uefi-protocol}
{
  ...
  description={a globally-uniquely identified shared data structure that is discoverable via \glsxtrshort{UEFI} services}
}
...
{
  name={Acidanthera \gls{UEFI} Development Kit},
...
\newglossaryentry{memory-permission}
{
...
{
  name={object file},
  description={an intermediate \gls{image-file} generated by a compiler from compilation units, which can be combined with other such into an \gls{executable-file} by a \gls{image-linker}}
}
...
{
  name={linking},
  description={a technique to enforce \glspl{object-file} and \glspl{library}} with each other or \glspl{executable-file}
}
...
\newglossaryentry{static-linking}
{
  ...
  description={a technique to combine multiple \glspl{object-file} into a \gls{library} or \gls{executable-file} at compile-time}
}

\newglossaryentry{dynamic-linking}
{
  name={dynamic \gls{linking}},
  description={a technique to load one or more \glspl{shared-library} into the \gls{address-space} of an executable at runtime}
}
...
\newglossaryentry{image-relocation}
{
  ...
  description={a technique to update all absolute address references of an \gls{image} from its \gls{image-base-address} to a new address}
}
...
\newglossaryentry{lazy-dynamic-linking}
{
  ...
  description={a technique to perform \gls{dynamic-linking} only on demand, e.g. when calling an externally imported function}
}

\newglossaryentry{RELROg}
{
  name={relocation read-only},
  description={a technique to enforce write protection on \glspl{image-segment} related to \gls{image-relocation} in \glsxtrshort{ELF} files}
}
...
{
  name={mmap},
  description={a function of UNIX-like operating systems to map files or devices into the \gls{address-space}}
}

\newglossaryentry{w-xor-x}
{
  name={W\^{}X},
  description={a technique to prohibit memory to be writable and executable at the same time~(pronounced `W XOR X')}
}

\newglossaryentry{XIPg}
{
  name={execute-in-place},
  description={a technique to execute a specially crafted \gls{executable-file} from its source storage without performing \gls{image-file-loading} or \gls{image-relocation} first}
}
...
\newglossaryentry{runtime-image-relocation}
{
  ...
  description={a technique to perform \gls{image-relocation} on an \gls{image} during its runtime when handing control over from \glsxtrshort{UEFI} to the operating system kernel}
}
...
{
  name={\glsxtrshort{UEFI-PI} Security},
...
{
  name={\glsxtrshort{UEFI-PI} Pre-EFI Initialization},
...
{
  name={\glsxtrshort{UEFI-PI} Driver Execution Environment},
...
{
  name={\glsxtrshort{UEFI-PI} Boot Device Search},
...
\newglossaryentry{UEFI-Boot-Manager}
{
  ...
  description={the \gls{UEFI} counterpart to \gls{BDS}, which handles \gls{HID} and boot policies}
}

\newglossaryentry{UEFI-Driver-Model}
{
  name={\gls{UEFI} Driver Model},
  description={the \gls{UEFI} design to support device drivers to probe and attach to hardware devices}
}

\newglossaryentry{TSLg}
{
  name={\glsxtrshort{UEFI-PI} Transient System Load},
...
\newglossaryentry{RTg}
{
  name={\glsxtrshort{UEFI-PI} Runtime},
...
\newglossaryentry{ALg}
{
  name={\glsxtrshort{UEFI-PI} Afterlife},
...
{
  name={Verified Boot},
  description={a technique to authenticate the entire platform \gls{firmware} boot process, typically with using a \gls{HRoT}}
}

\newglossaryentry{secure-boot}
{
  name={Secure Boot},
  description={a technique to authenticate \gls{UEFI} \gls{OS} loaders and third-party drivers using \glspl{digital-signature}}
}

\newglossaryentry{measured-boot}
{
  name={Measured Boot},
  description={a technique to collect critical information about the boot environment~(e.g. hardware information or \gls{firmware} configuration) and have a trusted remote attestation entity validate it}
}
...
\newglossaryentry{MMIOg}
{
  ...
  description={a technique to map device control registers into the \glsxtrshort{CPU} \gls{address-space}}
}
...
\newglossaryentry{memory-swapping}
{
  ...
  description={sometimes called paging, a technique to temporarily offload \gls{memory-page} onto secondary memory}
}
...
\newglossaryentry{PTIg}
{
  ...
  description={a technique to map only the minimal amount of \gls{kernel-space} \glspl{memory-page} into \gls{user-space} \glspl{address-space}}
}
...
\newglossaryentry{PEIM-shadowing}
{
  ...
  description={a technique to transfer an \glsxtrshort{XIP} \gls{image} from the \gls{firmware} storage to the main memory at runtime}
}

\newglossaryentry{CARg}
{
  name={cache-as-\glsxtrshort{RAM}},
  description={a technique to~(temporarily) use cache memory as if it was system \glsxtrshort{RAM}}
}
...
\newglossaryentry{image-preloading}
{
  ...
  description={a technique to perform \gls{image-file-loading} at build-time}
}

\newglossaryentry{image-prelinking}
{
  name={\gls{image} prelinking},
  description={a technique to perform \gls{dynamic-linking} at build-time}
}
...
\newglossaryentry{relative-addressing}
{
  ...
  description={a technique to encode addresses relative to the current \glsxtrshort{CPU} instruction}
}
...
\newglossaryentry{JITg}
{
  ...
  description={a technique to compile code, commonly a form of intermediate and abstract assembly-like language, just in time for its execution}
}
...
{
  name={return-to-libc},
  description={a technique to exploit programs by corrupting the \gls{call-stack} to overwrite the \gls{return-address} with a \gls{libc} address or another \gls{code-gadget}, typically to invoke shell commands}
}
...
{
  name={fuzz testing},
  description={a technique to automatically generate inputs for a computer program in order to test its behaviour, e.g. output correctness, internal invariant violations, or exceptions and crashes}
}
...
{
  name={copy-on-write},
  description={a technique to share data, which all consumers may independently modify, by lazily duplicating the source data only when the first write happens}
}
\newabbreviation{COW}{COW}{\gls{COWg}}

\newglossaryentry{UEFI-HIIg}
{
  name={\gls{UEFI} Human Interface Infrastructure},
  description={an \gls{UEFI} concept to manage user input and output, including forms, resources, and localization}
}
\newabbreviation{UEFI-HII}{\gls{UEFI} HII}{\gls{UEFI-HIIg}}

\newglossaryentry{module-dispatcher}
{
  name={\gls{UEFI-PI} module dispatcher},
  description={a phase-specific program that locates \glspl{PEIM}, \gls{UEFI}-related drivers, or \gls{MM} drivers and starts them with regard to their dependencies}
}

\newglossaryentry{CFIg}
{
  name={control flow integrity},
  description={a technique to enforce the integrity of the control flow, i.e. there are no branches to unexpected targets}
}
...
\newglossaryentry{static-memory-allocation}
{
  ...
  description={a technique to allocate fixed amounts of memory at compile-time, commonly utilizing a data \gls{image-segment}}
}

\newglossaryentry{call-stack-memory-allocation}
{
  name={\gls{call-stack} memory allocation},
  description={a technique to allocate and free memory at runtime (with the \gls{call-stack-frame} layout typically computed at compile-time) on the \gls{call-stack}}
}

\newglossaryentry{dynamic-memory-allocation}
{
  name={dynamic memory allocation},
  description={a technique to allocate and free arbitrary amounts of memory with a dynamic lifetime at runtime using \gls{heap-memory}}
}

\newglossaryentry{tail-recursion-elimination}
{
  name={tail-recursion elimination},
  description={a technique to optimize a recursive function call at the function's end to re-use the previous step's \gls{call-stack-frame}}
}
...
\newglossaryentry{OVMFg}
{
  ...
  description={an \glsxtrshort{Intel} architecture \gls{UEFI-PI} and \gls{UEFI} implementation by \gls{EDK2} for QEMU and \glsxtrshort{KVM}}
}
...
{
  name={ArmVirtQemu},
  description={an \glsxtrshort{Arm} architecture \gls{UEFI-PI} and \gls{UEFI} implementation by \gls{EDK2} for QEMU and  \glsxtrshort{KVM}}
}

\newglossaryentry{fault-injection}
{
  name={fault injection},
  description={a technique to test software with intentional faults to observe its error-handling}
}
...
\newglossaryentry{LTOg}
{
  ...
  description={a technique to move code generation to the link-time, which allows code optimization between code compilation units}
}
